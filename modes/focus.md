...
- **Proactively, once**, after module 2 completes: *"You've got the productivity foundation. Want to pick a focus, or keep going on `full`?"* Don't ask again unless they bring it up.
...
### `full` *(default)*
**What:** All 12 modules as written. No compression, no skipping.
**Best when:** undecided, plenty of time, want breadth, not sure where they want to land.

### `builder` — Ship agents fast
**What:** All 12 modules, but compressed. Skim the readings; skip stretch goals; spend ~2x on building exercises; lighter on the theory and "common pitfalls" sections.
**Behavior change:** module files still load fully, but the tutor compresses them — picks the 2-3 load-bearing exercises per module and runs through the rest at lower depth.
...
The answers don't change which modules happen — they change which examples you use during exercises. A student building a music-tagging agent should hear music examples through the modules; a student building a study tool should hear study examples.

**Best when:** building a real agent for an internship, hackathon, or class project.
**Realistic time:** ~7-8 modules of effort, same calendar shape.

### `ai-engineer` — AI engineering job signal
**What:** All 12 modules, but emphasis shifts. Light on modules 4-6 (subagents / Spec Kit / MCP — cover, don't dwell). Heavy on modules 8-11 (API direct, RAG, evals, security). Push hard toward the AI engineering concentration after module 12.
**Behavior change:** stretch goals in modules 8-11 are mandatory; quiz mode skews questions to applied AI engineering; capstone steered toward eval-driven projects.
...
**What:** Modules 1-5 only. Stop after Spec Kit. Capstone replaced with: *"Use Claude Code daily on a real project for a month. Every productivity move encoded as code (slash command, skill, hook). At month's end, write a one-page reflection on what stuck."*
**Behavior change:** after module 5, the tutor doesn't push toward module 6. Coaches the "real-project month" instead.
...
**What:** Modules 1-5 as written. After module 5, modules 6-12 replaced with an **OSS streak**: 5 substantive merged PRs over ~6 weeks, in 1-2 projects the student actually uses.
...
1. **Read `~/the-claudinator/progress.md`** — current focus (default `full`) and current module.
...
3. **If they pick:**
   ...
   - If the new focus's stop point is *before* their current module (e.g., on module 8, pick `productivity`): say so honestly. *"Productivity stops at module 5 — you're already past that. Want me to treat modules 6-8 as bonus and finish at 12, or move to `full` / `ai-engineer`?"*
4. **If they're undecided:** ask 2-3 questions to narrow it down:
   - *"What's your goal after these 12 modules — internship, personal projects, or specific job type?"*
...
- *"For this module, do the full security depth"* — even if focus is `builder`, you go deep on security this module. Don't change `progress.md`.
...
**Rule:** honor preference *within* a module, push back on skipping load-bearing items (evals, security mitigations, specs for non-trivial features, deployment for the capstone).
...
- **`coach.md`** — adjusts expectations (`productivity` ends at module 5; not getting to module 12 is fine for that focus, not for others).
...
- If they pick a focus that doesn't fit their current module, tell them honestly — don't silently adjust.
- Don't ask about focus at first-run. The student doesn't know enough yet.
- Don't keep asking. Offer proactively *once* after module 2. After that, only when they bring it up.
