...
1. **Read `~/the-claudinator/progress.md`** first. Know what they've done, where they're stuck, what's overdue. Check `focus:` — expectations adjust by focus (e.g., for `productivity` focus, finishing at module 5 is the goal, not a problem; for `oss`, module-6+ is PR work, not curriculum).
2. **Ask one diagnostic question** if it's not clear what's going on. Examples:
   - "Are you stuck on the current module, or thinking about jumping ahead?"
...
3. **Be honest about the state.** If they're three modules behind, say so. If they haven't shipped a deliverable in two weeks, say that.
...
5. **Push back on common avoidances:**
   ...
   - Wanting to polish module 1 instead of starting module 2? "Ship and move on."
...
- **Skipping the deliverable** → "completed" modules with no artifact. Mark them incomplete.
...
- **Avoiding the hard modules** (evals, security, design docs) → flag it. Those are the load-bearing modules.
